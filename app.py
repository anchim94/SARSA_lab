...
class MainApp(tk.Tk):
    def __init__(self):
        ...
        tk.Label(self, text="Katalog roboczy:").pack(anchor="w", padx=10, pady=(10, 0))
        frame_dir = tk.Frame(self)
        frame_dir.pack(fill="x", padx=10)
        tk.Entry(frame_dir, textvariable=self.workdir, width=50).pack(side="left", fill="x", expand=True)
        tk.Button(frame_dir, text="Wybierz...", command=self.choose_dir).pack(side="left", padx=5)

        # Przycisk uruchamiający uczenie
        tk.Button(self, text="Uruchom uczenie (air_qlearn)", command=self.run_qlearn, bg="#d0ffd0").pack(fill="x", padx=10, pady=(15, 5))

        # Wybór pliku do symulacji
        self.sim_file = tk.StringVar()
        tk.Label(self, text="Plik do symulacji:").pack(anchor="w", padx=10, pady=(10, 0))
        frame_file = tk.Frame(self)
        frame_file.pack(fill="x", padx=10)
        tk.Entry(frame_file, textvariable=self.sim_file, width=50).pack(side="left", fill="x", expand=True)
        tk.Button(frame_file, text="Wybierz plik...", command=self.choose_file).pack(side="left", padx=5)

        # Przycisk uruchamiający symulację
        tk.Button(self, text="Uruchom symulację (air_simul)", command=self.run_simul, bg="#d0d0ff").pack(fill="x", padx=10, pady=(15, 5))

        # Przycisk zamykający aplikację
        tk.Button(self, text="Zamknij", command=self.destroy, bg="#ffd0d0").pack(fill="x", padx=10, pady=(20, 10))
    ...
    def choose_file(self):
        filename = filedialog.askopenfilename(initialdir=self.workdir.get(), filetypes=[("Pliki pickle", "*.pkl"), ("Wszystkie pliki", "*.*")])
        if filename:
            self.sim_file.set(filename)

    def run_qlearn(self):
        os.chdir(self.workdir.get())
        p = multiprocessing.Process(target=qlearn_worker, args=(self.workdir.get(),))
        p.start()
        messagebox.showinfo("Info", "Uwaga! Możesz uruchomić kolejne przypadki równolegle.")
